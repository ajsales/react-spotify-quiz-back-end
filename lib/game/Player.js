...
export default class Player {
	...
	pickRandomSong(option) {
		const songList = this.songs[option];
		const index = getRandomInt(songList.length);
	...
	pickRandomArtist(option) {
		const artistList = this.artists[option];
		const index = getRandomInt(artistList.length);
