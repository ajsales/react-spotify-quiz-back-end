...
export default function QuestionFactory(top50) {
	...
	const IdentifyFavoriteSong = (players, option) => {
		...
		players = players.filter(player => player.likesAnySong(songs, option));
		...
		const questionObj = {
			...
			choices: songs,
			answers: songs.filter(song => player.likesSong(song, option)),
			song: song,
			img: player.img
		};
	...
	const IdentifyFavoriteArtist = (players, option) => {
		...
		let song = _getRandomArrEl(artist.songs);

		artists = artists.map(a => a.name);
		players = players.filter(player => player.likesAnyArtist(artists, option));
		let player = _getRandomArrEl(players);
		...
		const questionObj = {
			...
			highlight: [player.name],
			choices: artists,
			answers: artists.filter(a => player.likesArtist(a, option)),
			song: song,
			img: player.img
		}
	...
	const IdentifyPlayerFromSong = (players, option) => {
		...
		let answers = players.filter(player => player.likesSong(song.toString, option));
	...
	const IdentifyPlayerFromArtist = (players, option) => {
		...
		let answers = players.filter(player => player.likesArtist(artist.name, option));
		let choices = _getRandomPlayers(players, _getRandomArrEl(answers));
